...
def blog_index(request):
	...
	return render_to_response('blog/blog_index.html', {'archives': archives, 'categories': categories, 'posts': posts, 'comments': comments, 'links': links})
...
def blog_detail(request, blog_id):
	...
	archives = Archive.objects.all()
	# 取出最新的5条日志
	post_list = Post.objects.all().order_by('-id')[:5]
	return render_to_response('blog/blog_detail.html', {'post_list': post_list, 'archives': archives, 'categories': categories, 'posts': posts, 'type': 'blog', 'typeid': blog_id, 'comments': comments, 'links': links});
...
def blog_archive(request, archive_id, page):
	from django.core.paginator import Paginator
	categories = Category.objects.all()
	archive = Archive.objects.get(pk = archive_id)
	post_list = Post.objects.filter(archive = archive_id)
	pageSize = 10
	paginator = Paginator(post_list, pageSize)
	try:
		page = int(page)
